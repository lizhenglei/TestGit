...
+ (NSString *)base64StringFromText2:(NSString *)text
{
    if (text && ![text isEqualToString:LocalStr_None]) {
        //取项目的bundleIdentifier作为KEY
        //        NSString *key = [[NSBundle mainBundle] bundleIdentifier];
        ...
        //IOS 自带DES加密 Begin
       NSData * data3 = [self DESEncrypt:data2 WithKey:key];
        //IOS 自带DES加密 End
        return [self base64EncodedStringFrom:data3];
    }
    else {
        return LocalStr_None;
    }
...
+ (NSString *)textFromBase64String2:(NSString *)base64
{
    if (base64 && ![base64 isEqualToString:LocalStr_None]) {
        //取项目的bundleIdentifier作为KEY
//        NSString *key = [[NSBundle mainBundle] bundleIdentifier];
