...
@property(nonatomic,assign)int menuViewMidTag;//记录中间按钮返回后的底部按钮的tag
...
@property(nonatomic,assign)BOOL isMapPosition;//防止网点查询中两次调用位置的代理
@property(nonatomic,assign)BOOL isExitnegative;//判断是签退后的手势登录
@property(nonatomic,assign)BOOL isNativeLogin;//判断是签退后没有手势密码的登录
@property(nonatomic,assign)BOOL isSaoYiSao;//用于被动扫一扫后进入二维码转账界面，因为扫一扫不是加到菜单中的，需要单独判断
@property(nonatomic,assign)BOOL isOpenUrlBack;
//下面这些是????的东西
...
@property(nonatomic,strong)NSString *changeSkinColor;

@property(nonatomic,assign)BOOL MainAcSetBOOL;

//下面这四个参数是用于签退和超时登录退出后再从新登录，跳到之前打开的界面的参数(在webviewcontroller里赋的值)
@property(nonatomic,strong)NSString *toPassiveActionId;
@property(nonatomic,strong)NSString *toPassiveActionName;
@property(nonatomic,strong)NSString *toPassiveActionPrdId;
@property(nonatomic,strong)NSString *toPassiveActionToId;

@property(nonatomic,strong)NSString *toPassiveControllerString;//这个参数是记录被动登录的原生界面
@property(nonatomic,strong)NSString *rewardString;//用于VX判断是签到抽奖的字段
//@property(nonatomic,strong) CustomAlertView *login;

@property(nonatomic,strong)NSString *yuYueBranchName;
@property(nonatomic,strong)NSString *yuYueBranchId;
@property(nonatomic,strong)NSString *UserAnalysisActionId;//行为轨迹分析最后的actionid

@property(nonatomic,strong)NSString *shoudanLoginMessageString;//用于保存收单平台调原生登录成功后的信息
@property(nonatomic,strong)NSDictionary *zijinguijiInfo;//资金归集
