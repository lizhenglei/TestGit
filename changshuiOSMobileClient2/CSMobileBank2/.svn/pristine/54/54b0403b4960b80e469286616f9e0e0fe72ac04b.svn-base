...
@interface LogoutViewController ()<UIScrollViewDelegate,CSIIShareViewDelegate,WBHttpRequestDelegate,ShareHandleTencentDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,MobileSessionDelegate,CustomAlertViewDelegate>
{
    UIView *shareView;
    UILabel* titleLB;
...
- (void)viewDidLoad {
    ...
    [[MobileBankSession sharedInstance]postToServer:@"LiJinCoinQry.do" actionParams:nil method:@"POST"];
    ...
    UILabel *scoreLabel = [[UILabel alloc]initWithFrame:CGRectMake(titleLB.frame.origin.x, titleLB.frame.origin.y+titleLB.frame.size.height+15, 40, 20)];
    ...
    jifenScore = [[UILabel alloc]initWithFrame:CGRectMake(scoreLabel.frame.size.width+scoreLabel.frame.origin.x, scoreLabel.frame.origin.y, 80, 20)];
    jifenScore.font = [UIFont boldSystemFontOfSize:13];
    jifenScore.textColor = [UIColor whiteColor];
    jifenScore.textAlignment = NSTextAlignmentLeft;
    jifenScore.backgroundColor = [UIColor clearColor];
    [self.view addSubview:jifenScore];
    
    UIButton *scoreConvretBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    scoreConvretBtn.frame = CGRectMake(wight-45, titleLB.frame.origin.y+titleLB.frame.size.height+15, 40, 20);
    scoreConvretBtn.backgroundColor = [UIColor clearColor];
    scoreConvretBtn.titleLabel.font = [UIFont boldSystemFontOfSize:13];
    scoreConvretBtn.titleLabel.textColor = [UIColor whiteColor];
    [scoreConvretBtn setImage:[UIImage imageNamed:@"duihuan.png"] forState:UIControlStateNormal];
    [scoreConvretBtn addTarget:self action:@selector(scoreChangeClick) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:scoreConvretBtn];
    
    UIButton*exitBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    exitBtn.frame = CGRectMake(wight-80, titleLB.frame.origin.y+titleLB.frame.size.height+50, 72, 18);
    ...
    versionCode.backgroundColor = [UIColor clearColor];
...
-(void)scoreChangeClick
{
    [[WebViewController sharedInstance]setActionId:@"PChangeIntegral" actionName:@"积分兑换" prdId:@"PChangeIntegral" Id:@"PChangeIntegral"];
...
- (void)clickButton:(UIButton *)button withIndex:(NSInteger)index{
    ...
    if([[dict objectForKey:@"flag"] isEqualToString:@"0"]){
        ...
        if ([WXApi isWXAppInstalled] && [WXApi isWXAppSupportApi]){
            ...
            [handle messageToWeiXinNews:@"常熟农商银行" Description:@"常熟农商银行新版手机银行，转账0手续费、更有精彩活动等您参与，速来下载吧！" content:nil Image:[UIImage imageNamed:@"shareimage.png"] URL:@"http://www.csrcbank.com/download.html" shareScene:WXSceneTimeline];
        }else{
            ShowAlertView(@"提示", @"您尚未安装微信客户端", nil, @"确认", nil);
        }
...
-(void)getReturnData:(id)data WithActionName:(NSString *)action{
    if ([action isEqualToString:@"logout.do"]) {
        if ([[data objectForKey:@"_RejCode"]isEqualToString:@"000000"]) {
            ...
            [MobileBankSession sharedInstance].shoudanLoginMessageString = @"";//清除拉卡拉调用原生登录信息
            [MobileBankSession sharedInstance].isSaoYiSao = NO;//清除低优先级的东西
...
//            }
            if ([[data objectForKey:@"PlayRewards"]isEqualToString:@"1"]) {
                ...
                [MobileBankSession sharedInstance].rewardString = @"111111";//用于VX对签到抽奖的判断
                [[CSIIMenuViewController sharedInstance].navigationController pushViewController:[WebViewController sharedInstance] animated:NO];
                [[CSIIMenuViewController sharedInstance].drawerController toggleDrawerSide:XHDrawerSideRight animated:YES completion:nil];
                signUpBtn.selected = YES;
                return;
...
//            [alert show];
            if (signUpBtn.selected) {
                _signCalendarView = [[CustomAlertView alloc]initSignCalendarWithDelegate:self andSignData:data isSign:YES];
                [_signCalendarView show];
            }else{
                _signCalendarView = [[CustomAlertView alloc]initSignCalendarWithDelegate:self andSignData:data isSign:NO];
                [_signCalendarView show];
                signUpBtn.selected = YES;
    ...
    else if([action isEqualToString:@"LiJinCoinQry.do"]) {
        if ([[data objectForKey:@"_RejCode"]isEqualToString:@"000000"]) {
            if ([[data objectForKey:@"PointAvaile"]length]==0) {
                jifenScore.text = @"0";
            }else
            jifenScore.text = [data objectForKey:@"PointAvaile"];
        }
    }
}
