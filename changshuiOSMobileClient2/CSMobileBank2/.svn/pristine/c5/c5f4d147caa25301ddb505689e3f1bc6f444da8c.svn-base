    ...
    if (touch) {
        ...
        for (int i=0; i<buttonArray.count; i++) {
            ...
            if (CGRectContainsPoint(buttonTemp.frame,touchPoint)) {
                ...
                [buttonTemp setNeedsDisplay];
                CGRect frameTemp = buttonTemp.frame;
                CGPoint point = CGPointMake(frameTemp.origin.x+frameTemp.size.width/2,frameTemp.origin.y+frameTemp.size.height/2+8);
...
//    if (touchesArray.count<2)return;
    for (int i=0; i<touchesArray.count; i++) {
        ...
        if (success) {
//            CGContextSetRGBStrokeColor(context,0,0,0,0);//线条颜色      连接每个按钮的线,无色
            CGContextSetRGBStrokeColor(context,0.80f, 0.87f,0.72f, 0.50f);//线条颜色      连接每个按钮的线
        }
        else {
...
- (void)enterArgin {
    ...
    for (int i=0; i<buttonArray.count; i++) {
        GesturePasswordButton * buttonTemp = ((GesturePasswordButton *)[buttonArray objectAtIndex:i]);
        [buttonTemp setSelected:NO];
        buttonTemp.SecimgView.hidden = YES;
        [buttonTemp setSuccess:YES];
        [buttonTemp setNeedsDisplay];
    }
