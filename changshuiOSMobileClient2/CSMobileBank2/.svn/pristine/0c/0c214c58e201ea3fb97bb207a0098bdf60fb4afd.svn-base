...
//    swipView.backgroundColor = [UIColor redColor];
    ...
    backView = [[UIView alloc]initWithFrame:CGRectMake(10, 65, 100+10, 100+20)];
    backView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:backView];
    
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 103, 110, 15)];
    label.text = @"收款二维码";
    ...
    UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(titleLB.frame.origin.x, titleLB.frame.origin.y+titleLB.frame.size.height+5, wight-titleLB.frame.origin.x-20, 0.5)];
    lineView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:lineView];
    
    signUpBtn = [UIButton buttonWithType:UIButtonTypeCustom];
     signUpBtn.frame = CGRectMake(wight-60, backView.frame.origin.y+18, 80/2, 36/2);
    ...
    exitBtn.frame = CGRectMake(wight-90, titleLB.frame.origin.y+titleLB.frame.size.height+40, 72, 18);
...
-(void)Tapclick:(UITapGestureRecognizer*)tap
{
    ...
    if(Clickable!=nil && [Clickable isEqualToString:@"false"]){
        ...
        ShowAlertView(@"提示", @"功能暂不可用", nil, @"确定", nil);
        return;
    }
    ...
    if ([MobileBankSession sharedInstance].Userinfo!=nil&&[[[MobileBankSession sharedInstance].Userinfo objectForKey:@"UserType"]isEqualToString:@""]&&![RoleCtr isEqualToString:@""]) {    //P 大众版  T专业版    “”游客
        NSLog(@"无权限");
        ShowAlertView(@"提示", @"您无权限操作此功能，请到柜台开通专业版手机银行！", nil, @"确定", nil);
        return;
    }
    ...
    if ([MobileBankSession sharedInstance].Userinfo!=nil&&![[[MobileBankSession sharedInstance].Userinfo objectForKey:@"Security"]isEqualToString:@"05"]&&[RoleCtr isEqualToString:@"U"]) {    //P 大众版  T专业版    “”游客
        ...
        if ([string isEqualToString:@"1"]) {//0000101000
            ShowAlertView(@"提示", @"当前认证方式是“动态码＋交易密码”方式，请切换到“音频Key”方式！", nil, @"确定", nil);
            return;
        }
        else{
            NSLog(@"无权限");
            ShowAlertView(@"提示", @"您无权限操作此功能，请到柜台开通音频Key！", nil, @"确定", nil);
            return;
        }
    }
    ...
    if (((NSArray*)[nextMenu objectForKey:@"MenuList"]).count>0) {
        ...
        if ([[nextMenu objectForKey:@"ActionId"] isEqualToString:@"5000013"]) { //我要评价
            ...
            ShowAlertView(@"提示", @"功能完善", nil, @"确定", nil);
        }
    
    }else{
        ShowAlertView(@"提示", @"菜单有误", nil, @"确定", nil);
        return;
    }
}
...
-(void)signUpAction:(UIButton *)sender{
    ...
    if (sender.selected == NO) {
        ...
        ShowAlertView(@"提示", @"您已签到过，不可重复签到", nil, @"确定", nil);
        return;
    }
}
-(void)exit:(id)sender{
    ...
    if([[dict objectForKey:@"flag"] isEqualToString:@"0"]){
        ...
        if([WeiboSDK isCanSSOInWeiboApp]){
            if(![CSIIShareHandle SinaWeiBoTokenIsInvalid]){
                CSIIShareHandle *handle = [CSIIShareHandle ShareHandleInstance];
                //新闻信息
                WBMessageObject *messageObj = [handle messageToSinaShareNews:@"indentifier1" andTitle:@"常乐生活" Description:@"常熟农商行" ImgSmall:[UIImage imageNamed:@"icon7.png"] Url:@"http://www.csrcbank.com/download.html"];
                //图文片信息
//                WBMessageObject *messageObj = [handle messageToSinaShareWords:@"哈哈哈哈——————测试用得åå" andImg:[UIImage imageNamed:@"icon7"]];
        ...
        if ([WXApi isWXAppInstalled] && [WXApi isWXAppSupportApi]){
            ...
            [handle messageToWeiXinNews:@"常乐生活" Description:@"常熟农商行" content:nil Image:[UIImage imageNamed:@"icon7@2x.png"] URL:@"http://www.csrcbank.com/download.html" shareScene:WXSceneSession];
        }else{
            ShowAlertView(@"提示", @"您尚未安装微信客户端", nil, @"确定", nil);
        }
        
    }else if ([[dict objectForKey:@"flag"] isEqualToString:@"2"]){
        if ([WXApi isWXAppInstalled] && [WXApi isWXAppSupportApi]){
        handle.WXScene = WXSceneTimeline;
        [handle messageToWeiXinNews:@"常乐生活" Description:@"常熟农商行" content:nil Image:[UIImage imageNamed:@"icon7@2x.png"] URL:@"http://www.csrcbank.com/download.html" shareScene:WXSceneTimeline];
        }else{
            ShowAlertView(@"提示", @"您尚未安装微信客户端", nil, @"确定", nil);
        }
        ...
        if ([QQApi isQQInstalled]) {
            ...
            ShowAlertView(@"提示", @"您尚未安装腾讯QQ客户端", nil, @"确定", nil);
        }
...
- (void)sendSinaMessage{
    CSIIShareHandle *handle = [CSIIShareHandle ShareHandleInstance];
    WBMessageObject *messageObj = [handle messageToSinaShareWords:@"常熟农商行-" andImg:[UIImage imageNamed:@"sns_icon_1.png"]];
    //文字信息
    //WBMessageObject *messageObj = [handle messageToSinaShareOnlyWords:@"仅仅是文字信息的发布。____测试。"];
    WBSendMessageToWeiboRequest *request = [WBSendMessageToWeiboRequest requestWithMessage:messageObj authInfo:nil access_token:handle.SinaWBToken];
    [WeiboSDK sendRequest:request];
...
- (void)TencentLoginSuccess{
    ...
    CSIIShareHandle *handle = [CSIIShareHandle ShareHandleInstance];
    [handle messageToTencentNews:@"常乐生活" Description:@"常熟农商行" URL:@"http://www.csrcbank.com/download.html" PreviewImgData:imageData];
}
...
//            "_RejCode" = 000000;
            if ([[data objectForKey:@"PlayRewards"]isEqualToString:@"1"]) {
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"您已成功签到5天" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
                alert.tag = 10;
                [alert show];
                return;
            }
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:[NSString stringWithFormat:@"您已成功签到%@天",[data objectForKey:@"SeriesSignDay"]] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
        }
    }
   else{
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (alertView.tag ==10) {
        if (buttonIndex==1) {
            ShowAlertView(@"提示", @"恭喜你抽到一部iPhone6s", nil, @"确定", nil);
