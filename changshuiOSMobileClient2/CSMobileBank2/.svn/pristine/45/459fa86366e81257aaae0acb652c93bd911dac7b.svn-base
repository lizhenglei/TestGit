...
mockserver.handle = function (request) {
	...
	var param = {
		...
		"Url": request.method==="GET" ? "samples/" +request.url : request.url.replace("/pmbs/",""),
		"Data": data
	};
	window.NativeCall.sendRequest(request, JSON.stringify(param));
