...
-(void)rightSwipe:(UISwipeGestureRecognizer *)sgc
{
...
- (void)closeDrawerAnimated:(BOOL)animated completion:(void (^)(BOOL finished))completion {
    ...
    btn.tag=1002;
    [[CSIIMenuViewController sharedInstance]bottomButtonAction:btn];
}
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0) {
        [UIView animateWithDuration:0.5 delay:0.0 usingSpringWithDamping:damping initialSpringVelocity:velocity options:UIViewAnimationOptionAllowAnimatedContent animations:^{
            if (drawerSide == XHDrawerSideLeft) {
                ...
                [self.scrollView setContentOffset:CGPointMake(2 * XHContentContainerViewOriginX-54, 0.0f) animated:NO];//此处修改主控制器和右控制器的间距
            }
        } completion:^(BOOL finished) {
            self.openSide = drawerSide;
        ...
        [UIView animateWithDuration:0.3 delay:0 options:options animations:^{
            if (drawerSide == XHDrawerSideLeft) {
                ...
                [self.scrollView setContentOffset:CGPointMake(CGRectGetWidth(self.scrollView.frame) + XHContentContainerViewOriginX-90, 0.0f) animated:NO];
            }
...
- (void)setRightViewController:(UINavigationController *)rightViewController {
    if (![self isViewLoaded]) {
        [self view];
    }
