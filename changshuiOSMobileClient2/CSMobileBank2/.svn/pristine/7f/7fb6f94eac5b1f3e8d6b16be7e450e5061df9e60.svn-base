...
//                [button setBackgroundColor:[UIColor clearColor]];//当天日期的颜色
            }else
            {
                if (self.isSignCalen) {
//                    [button setBackgroundColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.66f alpha:1.00f]];
...
//            button.backgroundColor = [UIColor redColor];
            ...
            UIImageView *signImageView = [[UIImageView alloc]initWithFrame:CGRectMake(-2, button.frame.size.height-23, button.frame.size.width-4, 18)];
            signImageView.backgroundColor = [UIColor clearColor];
            signImageView.image = [UIImage imageNamed:@"signImage.png"];
...
//    CGColorSpaceRelease(baseSpace), baseSpace = NULL;
    ...
    UIView *bgView;
    if (self.isSignCalen) {
        bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 40, self.frame.size.width, 20)];
        bgView.backgroundColor = [UIColor colorWithRed:0.76f green:0.59f blue:0.53f alpha:1.00f];
        [self addSubview:bgView];
    }
    // Day labels
    __block CGRect frameWeekLabel = CGRectMake(0, 40, (self.frame.size.width-28)/7,20);
    __block CGRect frameWeekLabel22 = CGRectMake(0, 0, (self.frame.size.width-28)/7, 20);
    [_weekDayNames  enumerateObjectsUsingBlock:^(NSString * dayOfWeekString, NSUInteger idx, BOOL *stop)
     {
         ...
         frameWeekLabel22.origin.x       = (2+self.frame.size.width/7*idx);
         UILabel *weekNameLabel          = [[UILabel alloc] init];
         weekNameLabel.text              = dayOfWeekString;
         if (self.isSignCalen) {
             weekNameLabel.frame = frameWeekLabel22;
             weekNameLabel.textColor = [UIColor redColor];
             weekNameLabel.backgroundColor = [UIColor clearColor];
             [bgView addSubview:weekNameLabel];

         }else
         {
             weekNameLabel.frame = frameWeekLabel;
