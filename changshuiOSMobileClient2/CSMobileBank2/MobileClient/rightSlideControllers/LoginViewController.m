...
- (void)viewDidLoad {
    ...
    passwordTF = [[LWYTextField alloc] initWithFrame:CGRectMake(30, passTitle.frame.origin.y+passTitle.frame.size.height+5, 200, 30)];
    ...
    switch (btn.tag) {
        case LOGINBTN_TAG:
        {
            ...
            if (userNameTF.text.length==0) {
                ShowAlertView(@"提示", @"请输入手机号", nil, @"确认", nil);
                return;
            }
...
- (void)tagsAliasCallback:(int)iResCode
    ...
    NSLog(@"TagsAlias回调:  %d  %@   %@",iResCode,tags,alias);
}
-(void)getReturnData:(id)data WithActionName:(NSString *)action{
    ...
    if ([[data objectForKey:@"_RejCode"]isEqualToString:@"000000"]) {
        ...
        if ([action isEqualToString:@"login.do"]) {
            ...
            [APService setAlias:[data objectForKey:@"MobileNo"] callbackSelector:@selector(tagsAliasCallback:tags:alias:) object:self]; //极光推送设置别名
            ...
            if ([[data objectForKey:@"IsBind"] isEqualToString:@"N"]) {//首次登陆不需要绑定
...
//            }
            ...
            if ([[[MobileBankSession sharedInstance].Userinfo objectForKey:@"MainAcNo"] length]==0||[[[MobileBankSession sharedInstance].Userinfo objectForKey:@"MainAcNo"]isEqualToString:@""]) {
                ...
                NSArray *AcArray = [[NSArray alloc]initWithArray:[data objectForKey:@"AcList"]];
                for (int i=0; i<AcArray.count; i++) {
                    if (![[AcArray[i] objectForKey:@"AcType"]isEqualToString:@"4"]) {//有借记卡
                        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"您尚未设置主账户，请先设置主账户" delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil, nil];
                        alert.tag = 334;
                        [alert show];
                        LogoutViewController*logoutViewController = [[LogoutViewController alloc]init];
            ...
            if ([MobileBankSession sharedInstance].shoudanLoginMessageString.length>0) {
                
                [PiontStoreViewController sharedInstance].webViewUrl = [MobileBankSession sharedInstance].shoudanLoginMessageString;
                [[CSIIMenuViewController sharedInstance].navigationController pushViewController:[PiontStoreViewController sharedInstance] animated:YES];
...
//                [[WebViewController sharedInstance]setActionId:@"MyAcInfo" actionName:@"账户查询" prdId:@"MyAcInfo" Id:@"MyAcInfo"];
            ...
            if ([MobileBankSession sharedInstance].isPassiveLogin) {
                ...
                [[MobileBankSession sharedInstance] menuStartAction:nil];
                if ([MobileBankSession sharedInstance].isOpenUrlBack==YES) {
                    NSLog(@"is yes");
