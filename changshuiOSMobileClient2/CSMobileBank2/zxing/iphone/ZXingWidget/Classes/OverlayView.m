    ...
    if( self ) {
        ...
        if (!oneDMode) {
            if ([[UIScreen mainScreen] bounds].size.height <= 480.0f) {
                cropRect = CGRectMake(self.frame.size.width/2-100, (self.frame.size.height - rectSize) / 2+50, 200, 200);
            }else{
        ...
        if (self.bounds.size.height ==480) {
            imageV=[[UIImageView alloc]initWithFrame:CGRectMake((self.bounds.size.width-200)/2, (self.frame.size.height - rectSize)/2-49, 200, 2)];
        }else{
        imageV=[[UIImageView alloc]initWithFrame:CGRectMake(kPadding, (self.frame.size.height - rectSize)/2-49, rectSize, 2)];
        }
        //      imageV.image=[UIImage imageNamed:@"scan_line.png"];
        imageV.backgroundColor = [UIColor redColor];
        [self addSubview:imageV];
        CABasicAnimation *animation = [CABasicAnimation animation];
        animation.keyPath = @"position.y";
        animation.fromValue = [NSNumber numberWithFloat:((self.frame.size.height - rectSize)/2)];
        if (IPHONE5) {
            animation.toValue = [NSNumber numberWithFloat:(self.frame.size.height - rectSize)+130];
        }
        else{
            animation.toValue = [NSNumber numberWithFloat:(self.frame.size.height - rectSize)*2];
        }
        ...
        [imageV.layer addAnimation:animation forKey:@"basic"];
        ...
        //      } else {
        ////        [cancelButton setTitle:NSLocalizedStringWithDefaultValue(@"OverlayView cancel button title", nil, [NSBundle mainBundle], @"取消", @"Cancel") forState:UIControlStateNormal];
        //      }
...
- (void)cancel:(UIButton *)sender {
    ...
    if (delegate != nil) {
        [delegate cancelled];
        //        [delegate cancelledwith:(int)sender.tag];
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (displayedMessage == nil) {
        self.displayedMessage = NSLocalizedStringWithDefaultValue(@"OverlayView displayed message", nil, [NSBundle mainBundle], @"通过扫描他人收款二维码可进行转账", @"Place a barcode inside the viewfinder rectangle to scan it.");
    }
    ...
    if (oneDMode) {
        NSString *text = NSLocalizedStringWithDefaultValue(@"OverlayView 1d instructions", nil, [NSBundle mainBundle], @"通过扫描他人收款二维码可进行转账", @"Place a red line over the bar code to be scanned.");
        ...
        CGRect displayRect;
        if ([[UIScreen mainScreen] bounds].size.height <= 480.0f) {
            displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , cropRect.origin.y - displaySize.height-20, displaySize.width, displaySize.height);
        }else{
            displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , cropRect.origin.y - displaySize.height-10, displaySize.width, displaySize.height);
        }
